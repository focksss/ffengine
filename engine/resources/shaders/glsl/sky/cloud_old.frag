...
vec2 transmittance_lut_uv(float height, float cos_view_zenith) {
    float u = saturate((cos_view_zenith + 0.15) / 1.15); // Remap to handle below horizon
    float v = saturate((height - PLANET_RADIUS) / (ATMOSPHERE_RADIUS - PLANET_RADIUS));
    return vec2(u, v);
}

// Sample transmittance from LUT
vec3 get_transmittance_lut(vec3 pos, vec3 sun_dir) {
...
// Sample multiscattering from LUT
vec3 get_multiscatter_lut(float height, float cos_sun_zenith) {
    float u = saturate(cos_sun_zenith * 0.5 + 0.5);
    float v = saturate((height - PLANET_RADIUS) / (ATMOSPHERE_RADIUS - PLANET_RADIUS));
    return texture(atmosphere_multiscatter_lut, vec2(u, v)).rgb;
}
...
// Get atmospheric density at height
vec2 get_atmosphere_density(vec3 p) {
...
// Fast single-scatter approximation using LUTs
vec3 calculate_atmosphere_lut(vec3 ray_origin, vec3 ray_dir, float max_dist, vec3 sun_dir) {
    // Intersect with atmosphere
    float t_atmo_min, t_atmo_max;
    if (!intersect_sphere(ray_origin, ray_dir, PLANET_CENTER, ATMOSPHERE_RADIUS, t_atmo_min, t_atmo_max)) {
        return vec3(0.0);
    }

    // Intersect with planet surface
    float t_ground_min, t_ground_max;
    bool hit_ground = intersect_sphere(ray_origin, ray_dir, PLANET_CENTER, PLANET_RADIUS, t_ground_min, t_ground_max);

    t_atmo_min = max(0.0, t_atmo_min);
    t_atmo_max = min(t_atmo_max, max_dist);

    if (hit_ground && t_ground_min > 0.0) {
        t_atmo_max = min(t_atmo_max, t_ground_min);
    }

    float ray_length = t_atmo_max - t_atmo_min;
    if (ray_length <= 0.0) return vec3(0.0);

    float step_size = ray_length / float(ATMOSPHERE_IN_SCATTERING_STEPS);
    vec3 sample_point = ray_origin + ray_dir * (t_atmo_min + step_size * 0.5);
    ...
    // Get transmittance from camera to first sample point
    vec3 transmittance_camera = get_transmittance_lut(ray_origin, ray_dir);

    for (int i = 0; i < ATMOSPHERE_IN_SCATTERING_STEPS; i++) {
        vec2 density = get_atmosphere_density(sample_point);

        // Get transmittance from sample point to sun (LUT lookup replaces expensive raymarching!)
        vec3 transmittance_sun = get_transmittance_lut(sample_point, sun_dir);

        // Get multiscattering contribution
        float height = length(sample_point - PLANET_CENTER);
        float cos_sun_zenith = dot(normalize(sample_point - PLANET_CENTER), sun_dir);
        vec3 multiscatter = get_multiscatter_lut(height, cos_sun_zenith);

        // Accumulate scattering with LUT transmittance
        vec3 scatter_contrib = (transmittance_sun + multiscatter) * step_size;
        rayleigh_accum += density.x * scatter_contrib;
        mie_accum += density.y * scatter_contrib;
    ...
    return (rayleigh + mie) * SUN_INTENSITY * transmittance_camera;
}

// Simple transmittance lookup (replaces expensive raymarch)
vec3 get_atmosphere_transmittance(vec3 ray_origin, vec3 ray_dir, float dist) {
    float t_atmo_min, t_atmo_max;
    if (!intersect_sphere(ray_origin, ray_dir, PLANET_CENTER, ATMOSPHERE_RADIUS, t_atmo_min, t_atmo_max)) {
        return vec3(1.0);
    }
    ...
    t_atmo_min = max(0.0, t_atmo_min);
    t_atmo_max = min(t_atmo_max, dist);

    vec3 end_point = ray_origin + ray_dir * t_atmo_max;
    return get_transmittance_lut(end_point, -ray_dir);
}
...
float phase(float a) {
    ...
    float blend = 0.5;
    float hg_blend = henyey_greenstein(a, factors.x) * (1.0 - blend) + henyey_greenstein(a, -factors.y) * blend;
    return factors.z + hg_blend * factors.w;
}

// Modified lightmarch using LUTs (MUCH faster!)
vec3 lightmarch_with_atmosphere_lut(vec3 p, vec3 cloud_min, vec3 cloud_max, vec3 sun_dir) {
    vec3 d = sun_dir;
    float t_min = 0.0;
    float t_max = 0.0;
    ...
    float cloud_shadow = DARKNESS_THRESHOLD + exp(-accum_density * ABSORPTION_TOWARDS_SUN) * (1.0 - DARKNESS_THRESHOLD);

    // Use LUT for atmosphere lighting (replaces expensive calculate_atmosphere!)
    vec3 atmosphere_light = calculate_atmosphere_lut(p, sun_dir, 10000.0, sun_dir);

    // Sample night sky contribution

    return atmosphere_light * cloud_shadow;
}
...
void main() {
    ...
    vec3 sun_dir = normalize(SUN_DIR);
    float cos_theta = dot(d, sun_dir);
    float phase_value = phase(cos_theta);
    ...
    if (do_march) {
        ...
        while (t < travel_distance) {
            ...
            if (density > 0.0) {
                vec3 sample_energy = lightmarch_with_atmosphere_lut(p, cloud_min, cloud_max, sun_dir);
                energy += density * step_size * transmittance * sample_energy * phase_value;
    ...
    vec3 atmosphere_color = calculate_atmosphere_lut(o, d, max_atmosphere_dist, sun_dir);

    // Atmospheric transmittance using LUT
    vec3 atmosphere_transmittance_val = get_atmosphere_transmittance(o, d, t_geometry);
    ...
    if (!sky_hit) {
        final_color *= atmosphere_transmittance_val;
    }

    float final_alpha = sky_hit ? 1.0 : (1.0 - transmittance * atmosphere_transmittance_val.r);
