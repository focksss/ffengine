...
function update_angular_velocity_x_display()
    time_since_text_update = time_since_text_update + dt

	if time_since_text_update > 0.1 then
        gui.ActiveNode:get_text_at(0):update_text(string.format("%.3f", Engine.scene:get_rigid_body(_G.selected_rigid_body).angular_velocity.x))
        time_since_text_update = 0
        frame_count = 0
	end
end
function update_angular_velocity_y_display()
    time_since_text_update = time_since_text_update + dt

	if time_since_text_update > 0.1 then
        gui.ActiveNode:get_text_at(0):update_text(string.format("%.3f", Engine.scene:get_rigid_body(_G.selected_rigid_body).angular_velocity.y))
        time_since_text_update = 0
        frame_count = 0
	end
end
function update_angular_velocity_z_display()
    time_since_text_update = time_since_text_update + dt

	if time_since_text_update > 0.1 then
        gui.ActiveNode:get_text_at(0):update_text(string.format("%.3f", Engine.scene:get_rigid_body(_G.selected_rigid_body).angular_velocity.z))
        time_since_text_update = 0
        frame_count = 0
	end
end
